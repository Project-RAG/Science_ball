...
Create Elasticsearch index management and mappings for documents, chunks, entities, facts, and vectors.
...
- previous task files 001-011
- SDD Elasticsearch search/vector/numeric requirements
...
- Define mappings for documents, chunks, entities, facts.
- Include `access_level` in every searchable document.
- Represent numeric fields/ranges and nested numeric conditions.
- Make vector dimension configurable.
- Provide idempotent index creation/update helper.
...
- Mappings validate as plain dicts in tests.
- Index manager can be tested with mocked Elasticsearch client.
- `access_level` exists in all mappings.
...
- stubs/placeholders;
- known issues;
- next task: `TASK_013_indexing_chunks_and_facts.md`;
- commit readiness.
...
Текущая задача: `docs/tasks/TASK_012_elasticsearch_mappings.md`.
...
- `docs/tasks/TASK_012_elasticsearch_mappings.md`
...
- exact next task: `TASK_013_indexing_chunks_and_facts.md`;
...
4. что делать в следующей задаче `TASK_013_indexing_chunks_and_facts.md`.
```
