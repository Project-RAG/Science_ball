...
Implement MVP confidence scoring for extracted facts and entities.
...
- previous task files 001-010
- SDD confidence, evidence, LLM cap principle
...
- Define deterministic confidence formula based on extraction method, evidence quality, source type, normalization success.
- Represent rule that LLM-only extraction cannot exceed configured confidence cap.
- Store confidence inputs or explanation where practical.
- Keep scoring configurable and testable.
...
- change existing extracted facts broadly without tests
- change `docs/SDD.md`
...
- Tests cover deterministic, weak, and LLM-capped cases.
- Score explanation is available to API/service callers.
...
- stubs/placeholders;
- known issues;
- next task: `TASK_012_elasticsearch_mappings.md`;
- commit readiness.
...
Текущая задача: `docs/tasks/TASK_011_confidence_service.md`.
...
- `docs/tasks/TASK_011_confidence_service.md`
...
- exact next task: `TASK_012_elasticsearch_mappings.md`;
...
4. что делать в следующей задаче `TASK_012_elasticsearch_mappings.md`.
```
