...
Implement query understanding that converts user questions into validated search intent.
...
- previous task files 001-015
- SDD hybrid search filters, Russian/English queries
...
- Return validated Pydantic search intent with query text, filters, numeric constraints, language hints.
- Use deterministic fallback when LLM is disabled or fails.
- Use LLMGateway only, never provider directly.
- Validate and sanitize LLM output before use.
- Respect access_level as required input from caller/session context.
...
- bypass LLMGateway
- trust raw LLM JSON without validation
- change `docs/SDD.md`
...
- Tests cover deterministic fallback and mocked LLM success/failure.
- Invalid LLM output is rejected safely.
- Search intent schema is stable for search API.
...
- stubs/placeholders;
- known issues;
- next task: `TASK_017_search_api_basic.md`;
- commit readiness.
...
Текущая задача: `docs/tasks/TASK_016_query_understanding.md`.
...
4. что делать в следующей задаче `TASK_017_search_api_basic.md`.
```
