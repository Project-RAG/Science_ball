...
Extract numeric conditions and normalize units deterministically.
...
- previous task files 001-009
- SDD numeric conditions and deterministic extraction principle
...
- Extract temperature, pH, pressure, concentration, flow velocity, productivity, CAPEX/OPEX, recovery percent, year.
- Preserve raw value/unit/text and normalized value/unit.
- Attach source document/chunk/span metadata.
- Create candidate facts with deterministic extraction method metadata.
- Cover Russian and English notation where practical for MVP.
...
- write final reviewed facts without candidate/review status
...
- Unit tests cover main numeric types.
- Raw and normalized values are preserved.
- Facts are traceable to chunks.
- No invented values are created.
...
- stubs/placeholders;
- known issues;
- next task: `TASK_011_confidence_service.md`;
- commit readiness.
...
Текущая задача: `docs/tasks/TASK_010_numeric_extractor_and_units.md`.
...
- exact next task: `TASK_011_confidence_service.md`;
...
4. что делать в следующей задаче `TASK_011_confidence_service.md`.
```
