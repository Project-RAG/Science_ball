...
Create frontend MVP shell and document upload screen connected to backend upload API.
...
- previous task files 001-019
- SDD frontend stack React/TypeScript/Vite/Tailwind
...
- Initialize React + TypeScript + Vite app if frontend does not exist.
- Implement upload form with file, title, source_type, access_level, optional language/year.
- Call `POST /api/v1/documents/upload` through API client.
- Display upload success/error and returned document/job id.
...
- Upload UI sends multipart request.
- Errors are visible to user.
- No secrets are present in frontend files.
...
- stubs/placeholders;
- known issues;
- next task: `TASK_021_frontend_search_and_answer.md`;
- commit readiness.
...
Текущая задача: `docs/tasks/TASK_020_frontend_document_upload.md`.
...
- exact next task: `TASK_021_frontend_search_and_answer.md`;
...
4. что делать в следующей задаче `TASK_021_frontend_search_and_answer.md`.
```
